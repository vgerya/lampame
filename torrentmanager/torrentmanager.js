(function () {
                ...
                _startClient = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(client) {
                  ...
                  return _regeneratorRuntime().wrap(function _callee$(_context) {
                    while (1) switch (_context.prev = _context.next) {
                      case 0:
                        _context.t0 = client;
                        _context.next = _context.t0 === 'qBittorent'
                            ? 3
                            : _context.t0 === 'transmission'
                                ? 10
                                : _context.t0 === 'synology'
                                    ? 17
                                    :  _context.t0 === 'biglybt'
                                        ? 21
                                        : 18;
                      ...
                      case 21:
                        return _context.abrupt("return", BiglyBT.GetData());
                      case 18:
                    ...
                    onSelect: function onSelect(a) {
                      if (a.action === 'card') {
                        ...
                        switch (client) {
                          ...
                          case 'biglybt':
                            return BiglyBT.SendCommand(a, torrent_data);
                          default:
    ...
    function Main$1(manifest) {
      ...
      Lampa.SettingsApi.addParam({
        component: manifest.component,
        param: {
          name: manifest.component + "biglybtUrl",
          type: "input",
          //доступно select,input,trigger,title,static
          //values: `${Lampa.Storage.get("synologyUrl") || ""}`,
          placeholder: '',
        ...
        onRender: function onRender(item) {
          if (Lampa.Storage.field(manifest.component + 'Select') === "biglybt") {
            item.show();
          } else item.hide();
        },
        onChange: function onChange(item) {
      ...
      Lampa.SettingsApi.addParam({
        component: manifest.component,
        param: {
          name: manifest.component + "biglybtUser",
          type: "input",
          //доступно select,input,trigger,title,static
          placeholder: '',
          values: '',
      ...
      Lampa.SettingsApi.addParam({
        component: manifest.component,
        param: {
          name: manifest.component + "biglybtPass",
          type: "input",
          //доступно select,input,trigger,title,static
          placeholder: '',
          values: '',
      ...
      Lampa.SettingsApi.addParam({
        component: manifest.component,
        param: {
          name: manifest.component + "biglybtPathMovies",
          type: "input",
          //доступно select,input,trigger,title,static
          placeholder: '',
          values: '',
      ...
      Lampa.SettingsApi.addParam({
        component: manifest.component,
        param: {
          name: manifest.component + "biglybtPathTV",
          type: "input",
          //доступно select,input,trigger,title,static
          placeholder: '',
          values: '',
      ...
      Lampa.SettingsApi.addParam({
        ...
        param: {
          ...
          //доступно select,input,trigger,title,static
          "default": false,
          name: manifest.component + "biglybtProxy"
        },
        field: {
          name: Lampa.Lang.translate('LMEProxy'),
          description: Lampa.Lang.translate('LMEProxyDescription')
      ...
      // TODO BiglyBT tweak part...
    }


    // BiglyBT start
    var urlBiglyBT = Lampa.Storage.field("lmetorrentbiglybtUrl");
    var userBiglyBT = Lampa.Storage.field("lmetorrentbiglybtUser");
    var passBiglyBT = Lampa.Storage.field("lmetorrentbiglybtPass");
    var proxyBiglyBT = "";
    if (Lampa.Storage.field("lmetorrentbiglybtProxy") === true) {
        proxyBiglyBT = 'https://p01--corsproxy--h7ynqrkjrc6c.code.run/';
    }

    function getHeadersBiglyBT() {
        var headers = {
          Authorization: "Basic ".concat(btoa(userBiglyBT + ":" + passBiglyBT)),
          "Content-Type": "application/json"
        };
        if (Lampa.Storage.get("lmetorrentbiglybtSessionKey")) {
          headers["X-Transmission-Session-Id"] = Lampa.Storage.get("lmetorrentbiglybtSessionKey");
        }
        if (Lampa.Storage.field('lmetorrentbiglybtProxy') === true) {
          headers["x-requested-with"] = "lampame";
        }
    ...
    function AuthBiglyBT() {
      $.ajax({
            url: `${proxyBiglyBT}${urlBiglyBT}/transmission/rpc`,
            method: "GET",
            headers: getHeadersBiglyBT(),
            // Если getHeaders возвращает объект с ключом headers
            contentType: "application/json",
          success: function success(response, textStatus, jqXHR) {
            //resolve(Lampa.Noty.show(Lampa.Lang.translate('actionSentSuccessfully')));
          ...
          error: function error(jqXHR) {
            if (jqXHR.status === 409) {
              Lampa.Storage.set("lmetorrentbiglybtSessionKey", jqXHR.getResponseHeader("X-Transmission-Session-Id"));
              Lampa.Noty.show(Lampa.Lang.translate('BiglyBT AuthSuccess'));
            }
            if (jqXHR.status !== 200 && jqXHR.status !== 409) {
              console.log('LME Torrent manager', 'BiglyBT auth', jqXHR.status, jqXHR);
              Lampa.Noty.show(Lampa.Lang.translate('AuthDenied'));
            }
          }
      });
    ...
    function GetDataBiglyBT() {
       var request = {
            ...
            data: JSON.stringify({
                ...
                arguments: {
                  format: "objects",
                  fields: ["name", "id", "sizeWhenDone", "status", "percentDone"]
                }
            })
        };
        return new Promise((resolve, reject) => {
            $.ajax(request).done(response => {
                try {
                    var standardizedResponse = response.arguments.torrents.map(torrent => ({
                        name: torrent.name,
                        id: torrent.id,
                    ...
                    reject(new Error('Error processing data ' + response));
                }
            }).fail((jqXHR, textStatus, errorThrown) => {
                reject(new Error(`AJAX request error: ${textStatus} - ${errorThrown}`));
            });
        });
    }

    function SendTaskBiglyBT(selectedTorrent, labels) {
        var request = {
            url: `${proxyBiglyBT}${urlBiglyBT}/transmission/rpc`,
            method: "POST",
            headers: getHeadersBiglyBT(),
        ...
        return new Promise((resolve, reject) => {
            $.ajax(request).done(() => {
                resolve(Lampa.Noty.show("Action sent successfully"));
            }).fail((jqXHR, textStatus, errorThrown) => {
                reject(Lampa.Noty.show("Action returned an error"));
            });
        });
    }

    function SendCommandBiglyBT(btn, torrent_data) {
      var method;
      if (btn.action === 'resume') method = "torrent-start";
      if (btn.action === 'pause') method = "torrent-stop";
      if (btn.action === 'delete') method = "torrent-remove";
    ...
    var BiglyBT = {
        GetData: GetDataBiglyBT,
        SendTask: SendTaskBiglyBT,
        SendCommand: SendCommandBiglyBT,
        Auth: AuthBiglyBT
    };
    ...
    function Send() {
      ...
      function send2biglybt(selectedTorrent) {
        BiglyBT.SendTask(selectedTorrent, labels);
      }
      Lampa.Listener.follow('torrent', function (e) {
        if (e.type === 'onlong') {
          ...
          if (Lampa.Storage.field("lmetorrentSelect") === 'biglybt') {
            e.menu.push({
              title: "<div class=\"btnTDdownload wait\">\n                            <svg class=\"btnTDdownload\" viewBox=\"0 0 24 24\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\"><g id=\"SVGRepo_bgCarrier\" stroke-width=\"0\"></g><g id=\"SVGRepo_tracerCarrier\" stroke-linecap=\"round\" stroke-linejoin=\"round\"></g><g id=\"SVGRepo_iconCarrier\"> <path d=\"M8.5 7L8.5 14M8.5 14L11 11M8.5 14L6 11\" stroke=\"#ffffff\" stroke-width=\"1.5\" stroke-linecap=\"round\" stroke-linejoin=\"round\"></path> <path d=\"M15.5 7L15.5 14M15.5 14L18 11M15.5 14L13 11\" stroke=\"#ffffff\" stroke-width=\"1.5\" stroke-linecap=\"round\" stroke-linejoin=\"round\"></path> <path d=\"M18 17H12H6\" stroke=\"#ffffff\" stroke-width=\"1.5\" stroke-linecap=\"round\"></path> <path d=\"M2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2C16.714 2 19.0711 2 20.5355 3.46447C22 4.92893 22 7.28595 22 12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12Z\" stroke=\"#ffffff\" stroke-width=\"1.5\"></path> </g></svg>\n                            BiglyBT</div>",
              send2app: send2biglybt,
              onSelect: onSelectApp
            });
          }
          //Universal
    ...
    function add() {
      ...
      // Start BiglyBT Auth */
      if (Lampa.Storage.get(manifest.component + 'Select') === 'biglybt') {
        BiglyBT.Auth()
      }
    }
